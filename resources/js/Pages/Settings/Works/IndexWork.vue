<script setup>
    ...
    let searchUrl = computed(() => {
        let url = new URL(route('works.index'));
    ...
    const ubahData = () => {
        form.put(route('works.update', form.id), {
            onSuccess: () => {
                form.reset();
                form.clearErrors();
                showModalUpdate.value = false;
    ...
    const hapusData = () => {
        form.delete(route('works.destroy', form.id), {
            onSuccess: () => {
                form.reset();
                form.clearErrors();
                showModalDelete.value = false;
    ...
    <AuthenticatedLayout>
        <div class="grid grid-cols-1 h-full">
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                        ...
                        <li aria-current="page">
                            <div class="flex items-center">
                                ...
                                <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Pekerjaan</span>
                            </div>
                        </li>
                    </ol>
                </nav>
            ...
            <div class="flex justify-between my-3">
                ...
                <div>
                    <template v-if="hasPermission('work: create')">
                        ...
                        <Modal :show="showModalCreate" @close="closeModalCreate">
                            <div class="relative w-full max-w-3xl max-h-full bg-white rounded-lg shadow dark:bg-gray-700">
                                ...
                                <form @submit.prevent="tambahData">
                                    <div class="px-4 py-2">
                                        <div class="mb-2">
                                            <label for="work_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Pekerjaan</label>
                                            <input v-model="form.work_name" type="text" id="work_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Masukkan nama pekerjaan" />
                                            <InputError :message="form.errors.work_name" />
                                        </div>
                                    </div>
            ...
            <Table>
                <template #header>
                    <TableRow>
                        ...
                        <TableHeaderCell>NAMA PEKERJAAN</TableHeaderCell>
                ...
                <template #default>
                    <TableRow v-for="(data, index) in fetchData.data" :key="data.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <TableDataCell :status="'record'">{{ data.work_name }}</TableDataCell>
                        ...
                        <TableDataCell :status="'action'">
                            <!-- Lihat Data  -->
                            <template v-if="hasPermission('work: read')">
                            ...
                            <!-- Ubah Data  -->
                            <template v-if="hasPermission('work: update')">
                            ...
                            <!-- Hapus Data  -->
                            <template v-if="hasPermission('work: delete')">
