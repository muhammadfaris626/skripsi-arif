<script setup>
    ...
    let searchUrl = computed(() => {
        let url = new URL(route('certificate-categories.index'));
    ...
    const modalLiatData = (data) => {
        ...
        form.id = data.id;
        form.certificate_category_name = data.certificate_category_name;
    }
    ...
    const tambahData = () => {
        form.post(route('certificate-categories.store'), {
            onSuccess: () => {
                form.reset();
                form.clearErrors();
                showModalCreate.value = false;
    ...
    const ubahData = () => {
        form.put(route('certificate-categories.update', form.id), {
            onSuccess: () => {
                form.reset();
                form.clearErrors();
                showModalUpdate.value = false;
    ...
    const hapusData = () => {
        form.delete(route('certificate-categories.destroy', form.id), {
            onSuccess: () => {
                form.reset();
                form.clearErrors();
                showModalDelete.value = false;
    ...
    <AuthenticatedLayout>
        <div class="grid grid-cols-1 h-full">
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                        ...
                        <li aria-current="page">
                            <div class="flex items-center">
                                ...
                                <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Kategori Surat</span>
                            </div>
                        </li>
                    </ol>
                </nav>
            ...
            <div class="flex justify-between my-3">
                ...
                <div>
                    <template v-if="hasPermission('certificate-category: create')">
                        ...
                        <Modal :show="showModalCreate" @close="closeModalCreate">
                            <div class="relative w-full max-w-3xl max-h-full bg-white rounded-lg shadow dark:bg-gray-700">
                                ...
                                <form @submit.prevent="tambahData">
                                    <div class="px-4 py-2">
                                        <div class="mb-2">
                                            <label for="certificate_category_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Kategori Surat</label>
                                            <input v-model="form.certificate_category_name" type="text" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Masukkan nama kategori surat" />
                                            <InputError :message="form.errors.certificate_category_name" />
                                        </div>
                                    </div>
            ...
            <Table>
                <template #header>
                    <TableRow>
                        ...
                        <TableHeaderCell>NAMA KATEGORI SURAT</TableHeaderCell>
                ...
                <template #default>
                    <TableRow v-for="(data, index) in fetchData.data" :key="data.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <TableDataCell :status="'record'">{{ data.certificate_category_name }}</TableDataCell>
                        ...
                        <TableDataCell :status="'action'">
                            <!-- Lihat Data  -->
                            <template v-if="hasPermission('certificate-category: read')">
                            ...
                            <!-- Ubah Data  -->
                            <template v-if="hasPermission('certificate-category: update')">
                            ...
                            <!-- Hapus Data  -->
                            <template v-if="hasPermission('certificate-category: delete')">
                                ...
                                <table class="table-collapse w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    <tbody>
                                        ...
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                            ...
                                            <td class="px-6 py-4">
                                                {{ form.certificate_category_name }}
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
