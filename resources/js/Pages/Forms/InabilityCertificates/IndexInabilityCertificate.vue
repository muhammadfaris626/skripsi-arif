<script setup>
    ...
    let searchUrl = computed(() => {
        let url = new URL(route('inability-certificates.index'));
    ...
    const modalLiatData = (data) => {
        ...
        form.id = data.id;
        form.cover_letter_id = data.cover_letter_id[0].request_number;
        form.request_number = data.request_number;
        form.user_id = data.user_id[0].name;
        form.name = data.name;
        ...
        form.validity_period = data.validity_period;
    ...
    const ubahData = () => {
        form.put(route('inability-certificates.update', form.id), {
            onSuccess: () => {
                form.reset();
                form.clearErrors();
    ...
    const hapusData = () => {
        form.delete(route('inability-certificates.destroy', form.id), {
            onSuccess: () => {
                form.reset();
                form.clearErrors();
                showModalDelete.value = false;
    ...
    const cetak = () => {
        form.get(route('inability-certificates.show', form.id), {
            onSuccess: () => {
                form.reset();
                form.clearErrors();
                showModalRead.value = false;
...
<template>
    <Head title="Surat Keterangan Tidak Mampu" />
    <AuthenticatedLayout>
        <div class="grid grid-cols-1 h-full">
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                        ...
                        <li aria-current="page">
                            <div class="flex items-center">
                                ...
                                <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Surat Keterangan Tidak Mampu</span>
                            </div>
                        </li>
                    </ol>
                </nav>
            ...
            <div class="flex justify-between my-3">
                ...
                <div>
                    <template v-if="hasPermission('inability-certificate: create')">
                        <Link :href="route('inability-certificates.create')" class="px-5 py-2.5 text-sm font-medium text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
            ...
            <Table>
                <template #header>
                    <TableRow>
                        ...
                        <TableHeaderCell>NOMOR SURAT PENGANTAR</TableHeaderCell>
                ...
                <template #default>
                    <TableRow v-for="(data, index) in fetchData.data" :key="data.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <TableDataCell :status="'record'">{{ data.cover_letter_id[0].request_number }}</TableDataCell>
                        ...
                        <TableDataCell :status="'action'">
                            <!-- Lihat Data  -->
                            <template v-if="hasPermission('inability-certificate: read')">
                            ...
                            <!-- Ubah Data  -->
                            <template v-if="hasPermission('inability-certificate: update')">
                            ...
                            <!-- Hapus Data  -->
                            <template v-if="hasPermission('inability-certificate: delete')">
                ...
                <div class="relative w-full max-w-3xl max-h-full bg-white rounded-lg shadow dark:bg-gray-700">
                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                            LIHAT SURAT KETERANGAN TIDAK MAMPU
                        </h3>
                        <div v-if="allApproved">
                            <button v-if="new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) > new Date(form.validity_period).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })" @click="cetak" type="button" class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2 text-center inline-flex items-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3" />
                                </svg>
                                UNDUH
                            </button>
                            <button v-else disabled type="button" class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2 text-center inline-flex items-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">
                                EXPIRED
                            </button>
                        </div>
                        <div v-else>
                            <button disabled type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                SEDANG DIPROSES
                            </button>
                        </div>
                    </div>
                            ...
                            <table class="table-collapse w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                <tbody>
                                    ...
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td class="px-6 py-4">
                                            {{ new Date(form.validity_period).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) }}
                                        </td>
                                    </tr>
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                ...
                <div class="relative w-full max-w-3xl max-h-full bg-white rounded-lg shadow dark:bg-gray-700">
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                            Apakah anda yakin ingin menghapus surat keterangan tidak mampu ini ?
                        </h3>
